...
async function main() {
    const passwordHash = await (0, password_1.hashPassword)("CareConcern@123");
    await prisma.user.upsert({ where: { email: "admin@careconcern.in" }, update: {}, create: { email: "admin@careconcern.in", phone: "9000000001", fullName: "Care Concern Admin", role: client_1.Role.ADMIN, passwordHash } });
    await prisma.user.upsert({ where: { email: "reception@careconcern.in" }, update: {}, create: { email: "reception@careconcern.in", phone: "9000000002", fullName: "Reception Desk", role: client_1.Role.RECEPTIONIST, passwordHash } });
    ...
    for (const doctor of doctors) {
        ...
        const user = await prisma.user.upsert({
            ...
            update: {},
            create: { email: `${doctor.publicId}@careconcern.in`, fullName: doctor.fullName, role: client_1.Role.DOCTOR, passwordHash }
        });
